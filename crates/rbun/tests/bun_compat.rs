...
fn run_suite() -> Result<(), String> {
    ...
    let reference = std::env::var_os("RBUN_REFERENCE_BUN")
        .map(PathBuf::from)
        .unwrap_or_else(|| repo.join("com/github/oven-sh/bun/dist/build/release/bun"));
...
fn verify_reference_revision(reference: &Path, repo: &Path) -> Result<(), String> {
    let source = repo.join("com/github/oven-sh/bun/src");
    let expected_output = Command::new("git")
        .arg("-C")
        .arg(&source)
        .args(["rev-parse", "HEAD"])
        .output()
        .map_err(|error| format!("read Bun submodule revision: {error}"))?;
    if !expected_output.status.success() {
        return Err(format!(
            "cannot read Bun submodule revision from {}: {}",
            source.display(),
            String::from_utf8_lossy(&expected_output.stderr).trim()
        ));
    }
    ...
    {
        return Err(format!(
            "reference Bun is not built from source submodule {expected}: {:?}",
            revision.trim()
        ));
    }
    Ok(())
