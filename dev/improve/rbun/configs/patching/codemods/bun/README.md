...
JSSG codemod that makes Bun's Rust runtime (`bun_runtime`, Bun >= 1.4)
embeddable by rbun. Applied to the generated Bun `dist/` by
`com/github/oven-sh/bun/_vendor.ts`; see the parent README for the commands.
...
| `files/scripts/embed-dylib.ts` (new) | Relinks the executable's link edge as `libbun_embed.dylib` exporting `bun_embed_*`, the JavaScriptCore C API and Bun's `symbols.txt`. | The artifact rbun loads (`com/github/oven-sh/bun/dist/build/release/libbun_embed.dylib`). |
