#!/usr/bin/env bun
/**
 * Synchronize, classify, and differentially run Bun's upstream runtime tests
 * against the exact commit embedded by rbun.
 *
 ...
 *
 * `RBUN_UPSTREAM_BUN_DIR` may point at an existing full/sparse Bun checkout.
 */
...
const revision = readFileSync(join(repo, "vendor/bun/VENDORED_COMMIT"), "utf8").trim();
const config = JSON.parse(readFileSync(join(repo, "compat/upstream-suites.json"), "utf8"));
const checkout = resolve(process.env.RBUN_UPSTREAM_BUN_DIR ?? join(repo, "target/bun-upstream-tests"));
const testRoot = join(checkout, "test");
const reference = resolve(process.env.RBUN_REFERENCE_BUN ?? join(repo, "vendor/bun/build/release/bun"));
...
function printUsage(): void {
  ...
  bun scripts/bun-upstream-tests.ts sync
  bun scripts/bun-upstream-tests.ts classify
  bun scripts/bun-upstream-tests.ts run <suite> [substring]
...
function syncCheckout(): void {
  ...
  const actual = runCommand(["git", "-C", checkout, "rev-parse", "HEAD"], repo, true).trim();
  if (actual !== revision) throw new Error(`upstream checkout is ${actual}, expected ${revision}`);
}

function requireCheckout(): void {
  if (!existsSync(testRoot)) {
    throw new Error(`upstream tests are missing; run: bun scripts/bun-upstream-tests.ts sync`);
  }
  const actual = runCommand(["git", "-C", checkout, "rev-parse", "HEAD"], repo, true).trim();
  if (actual !== revision) {
