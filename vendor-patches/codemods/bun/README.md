...
| `files/src/runtime/embed.rs` (new) | `bun_embed_*` extern "C" surface: process init, per-thread VM creation (holding the JSC API lock), event-loop ticking, promise status/result helpers, module-registry eviction, GC, last-error. | The surface `rbun::ffi` links against. |
